...
\justify
\only<1>{\myheading{Atari}}
\only<2>{\myheading{Go}}
\only<3>{\myheading{Poker}}
\only<4>{\myheading{Defense of the Ancients}}
\only<1>{\begin{itemize}
		\item{Scale to difficult Reinforcement Learning problems using neural networks as function approximators} 
		\item{
...
\only<2>{\begin{itemize}
		...
		\end{itemize}}
\only<3>{DeepStack defeated 11 professional poker players with only one outside the margin of statistical significance.}
...
\only<3>{\includegraphics[scale=0.1]{images/poker}}
\only<4>{\includegraphics[scale=0.1]{images/dota}}
\end{figure}
\end{overlayarea}
\end{columns}
\end{minipage}
