...
\draw[thin,|-|,xshift=3pt] (start-|end) -- node[black,fill=white,right] {#4}(end); 
  ...
  \caption{Biological Neurons$^*$\footnotetext{$^*$Image adapted from \url{https://cdn.vectorstock.com/i/composite/12,25/neuron-cell-vector-81225.jpg}}}
  \end{figure}
  }
  \end{overlayarea}
...
\onslide<1->{Sample illustration of hierarchical processing \\~ \footnote{Idea borrowed from Hugo Larochelle's lecture slides}}
...
\end{tikzpicture}
\captionof{figure}{$x_1$ AND $!x_2$ \footnote{circle at the end indicates inhibitory input: if any inhibitory input is 1 the output will be 0}}
}
\end{center}
...
\draw [->] (input3) -- (neuron1);
\draw [->] (input4) -- (neuron1);
\onslide<11->{\draw [->, red] (input5) -- (neuron1);}
...
%\node (formula)[scale=.8] at (10.4,0.6) {$..$};
\node (formula)[scale=.8] at (11.1,0.6) {$w_{3}$};
...
\begin{frame}
Let us inspect this a bit more carefully ... 
\end{frame}
...
\item<3-> What is wrong with this line? \only<4->{We make an error on 1 out of the 4 inputs}
...
\item If the threshold be 0.5 and $w_1 = 1$ then what would be the decision for a movie with $criticsRating=0.51$ ? \onslide<5->{watch}
\item What about a movie with $criticsRating=0.49$ ? \onslide<6->{don't watch}
